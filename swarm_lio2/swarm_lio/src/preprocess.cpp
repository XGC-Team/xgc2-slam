#include "preprocess.h"

// 返回值定义：用于判断Livox点云的回波类型
#define RETURN0     0x00       // 第0个回波
#define RETURN0AND1 0x10       // 第0和第1个回波

// 按时间戳（curvature字段存储）对点云进行排序的比较函数
const bool time_list_cut_frame(PointType &x, PointType &y) {
    return (x.curvature < y.curvature);
}

/**
 * @brief 预处理类构造函数，初始化所有参数
 *
 * 初始化特征提取相关的所有阈值参数，包括距离阈值、角度阈值等
 */
Preprocess::Preprocess()
        : feature_enabled(0), lidar_type(AVIA), blind(1.0), DET_RANGE(100.0), point_filter_num(1) {
    inf_bound = 10;              // 无穷远边界阈值
    N_SCANS = 6;                 // 默认扫描线数（Livox Avia为6线）
    group_size = 8;              // 平面检测的点群大小
    disA = 0.01;                 // 距离系数A
    disA = 0.1; // B?            // 距离系数B（此处应为disB）
    p2l_ratio = 225;             // 点到线距离比率
    limit_maxmid = 6.25;         // 最大距离与中间距离的比率限制
    limit_midmin = 6.25;         // 中间距离与最小距离的比率限制
    limit_maxmin = 3.24;         // 最大距离与最小距离的比率限制
    jump_up_limit = 170.0;       // 向上跳跃角度限制（度）
    jump_down_limit = 8.0;       // 向下跳跃角度限制（度）
    cos160 = 160.0;              // 边缘角度阈值（度）
    edgea = 2;                   // 边缘检测参数a
    edgeb = 0.1;                 // 边缘检测参数b
    smallp_intersect = 172.5;    // 小平面交角阈值（度）
    smallp_ratio = 1.2;          // 小平面距离比率阈值
    given_offset_time = false;   // 是否给定偏移时间标志

    // 将角度转换为余弦值，用于后续快速计算
    jump_up_limit = cos(jump_up_limit / 180 * M_PI);
    jump_down_limit = cos(jump_down_limit / 180 * M_PI);
    cos160 = cos(cos160 / 180 * M_PI);
    smallp_intersect = cos(smallp_intersect / 180 * M_PI);
}

/**
 * @brief 预处理类析构函数
 */
Preprocess::~Preprocess() {}

/**
 * @brief 设置预处理参数
 * @param feat_en 是否启用特征提取
 * @param lid_type 激光雷达类型
 * @param bld 盲区距离
 * @param pfilt_num 点滤波数量
 */
void Preprocess::set(bool feat_en, int lid_type, double bld, int pfilt_num) {
    feature_enabled = feat_en;
    lidar_type = lid_type;
    blind = bld;
    point_filter_num = pfilt_num;
}

/**
 * @brief 处理Livox激光雷达数据
 * @param msg Livox激光雷达消息
 * @param pcl_out 输出的处理后点云（平面点云）
 */
void Preprocess::process(const livox_ros_driver::CustomMsg::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out) {
    avia_handler(msg);
    *pcl_out = pl_surf;
}

/**
 * @brief 处理Livox激光雷达数据并进行分帧切割
 * @param msg Livox激光雷达消息
 * @param pcl_out 输出的点云队列（多帧）
 * @param time_lidar 每帧的时间戳队列
 * @param required_frame_num 需要切割的帧数
 * @param scan_count 当前扫描计数（用于判断是否在初始化阶段）
 */
void Preprocess::process_cut_frame_livox(const livox_ros_driver::CustomMsg::ConstPtr &msg,
                                         deque<PointCloudXYZI::Ptr> &pcl_out, deque<double> &time_lidar,
                                         const int required_frame_num, int scan_count) {
    int plsize = msg->point_num;
    pl_surf.clear();
    pl_surf.reserve(plsize);
    pl_full.clear();
    pl_full.resize(plsize);

    // 遍历所有点，进行滤波和预处理
    for (uint i = 1; i < plsize; i++) {
        // 仅保留第0个回波（单次反射），过滤多次反射点
        if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10) ||
            (msg->points[i].tag & 0x30) == 0x00) //保留第0个回波
        {
            pl_full[i].x = msg->points[i].x;
            pl_full[i].y = msg->points[i].y;
            pl_full[i].z = msg->points[i].z;
            pl_full[i].intensity = msg->points[i].reflectivity;
            // 使用curvature字段存储每个点的时间偏移，单位：毫秒
            pl_full[i].curvature = msg->points[i].offset_time / float(1000000);

            // 检查时间偏移是否超出合理范围（激光雷达周期+20ms容差）
            if(pl_full[i].curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << pl_full[i].curvature/1000 << RESET << endl;
                continue;
            }

            // 计算点到激光雷达的距离
            double dist = pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z;
            // 过滤盲区内的点和超出最大检测距离的点
            if (dist < blind * blind || dist > DET_RANGE * DET_RANGE) continue;

//            double yaw_angle = atan2(pl_full[i].y, pl_full[i].x) * 57.3;
//            if((yaw_angle < -160 || yaw_angle > 160) && (cur_time - first_time) > 90)
//                continue;

            // 去除重复点：检查当前点与前一个点的坐标是否相同
            if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7)
                || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7)
                || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) {
                pl_surf.push_back(pl_full[i]);
            }
        }
    }

    // 对点云按时间进行排序（时间存储在curvature字段中）
    sort(pl_surf.points.begin(), pl_surf.points.end(), time_list_cut_frame);

    // 分割的上一帧的帧末时间，单位ms
    double last_frame_end_time = msg->header.stamp.toSec() * 1000;
    uint valid_num = 0;
    uint cut_num = 0;
    uint valid_pcl_size = pl_surf.points.size();

    // 初始化阶段（前20帧）只分割为1帧，之后按需要分割
    int required_cut_num = required_frame_num;
    if (scan_count < 20)
        required_cut_num = 1;

    // 暂时存储分割后的帧，存入pcl_temp后清空
    PointCloudXYZI pcl_cut;
    // 分帧处理
    for (uint i = 1; i < valid_pcl_size; i++) {
        valid_num++;
        // 计算新的偏移时间（相对于切割后的帧头）：单位ms
        pl_surf[i].curvature += msg->header.stamp.toSec() * 1000 - last_frame_end_time;
        pcl_cut.push_back(pl_surf[i]);

        // 当累积的点数达到一帧的大小时，保存当前帧
        if (valid_num == (int((cut_num + 1) * valid_pcl_size / required_cut_num) - 1)) {
            cut_num++;
            time_lidar.push_back(last_frame_end_time);

            // pcl_temp用于临时存储pcl_cut的值
            PointCloudXYZI::Ptr pcl_temp(new PointCloudXYZI()); //shared_ptr一定要初始化
            *pcl_temp = pcl_cut;

            pcl_out.push_back(pcl_temp);
            // 更新帧头（上一帧帧末时间）
            last_frame_end_time += pl_surf[i].curvature;
            // 清空pcl_cut，准备存储下一帧
            pcl_cut.clear();
            pcl_cut.reserve(valid_pcl_size * 2 / required_frame_num);
        }
    }
}

/**
 * @brief 处理PointCloud2格式的激光雷达数据并进行分帧切割
 * @param msg PointCloud2格式的激光雷达消息
 * @param pcl_out 输出的点云队列（多帧）
 * @param time_lidar 每帧的时间戳队列
 * @param required_frame_num 需要切割的帧数
 * @param scan_count 当前扫描计数（用于判断是否在初始化阶段）
 */
void
Preprocess::process_cut_frame_pcl2(const sensor_msgs::PointCloud2::ConstPtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out,
                                   deque<double> &time_lidar, const int required_frame_num, int scan_count) {
    pl_surf.clear();
    pl_full.clear();
    // 根据激光雷达类型选择不同的数据转换方式
    if (lidar_type == VELO) {
        // 处理Velodyne激光雷达数据
        pcl::PointCloud<velodyne_ros::Point> pl_orig;
        pcl::fromROSMsg(*msg, pl_orig);
        int plsize = pl_orig.points.size();
        pl_surf.reserve(plsize);
        for (int i = 0; i < plsize; i++) {
            PointType added_pt;
            added_pt.normal_x = 0;
            added_pt.normal_y = 0;
            added_pt.normal_z = 0;
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature = pl_orig.points[i].time * 1000.0;  // 转换为毫秒

            // 检查时间偏移是否超出合理范围
            if(added_pt.curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << added_pt.curvature/1000 << RESET << endl;
                continue;
            }

            // 仅保留有效线数和盲区外的点
            if (pl_orig.points[i].ring < N_SCANS) {
                if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > blind * blind)
                    pl_surf.points.push_back(added_pt);
            }
        }
    } else if (lidar_type == OUSTER) {
        pcl::PointCloud<ouster_ros::Point> pl_orig;
        pcl::fromROSMsg(*msg, pl_orig);
        int plsize = pl_orig.points.size();
        pl_surf.reserve(plsize);
        for (int i = 0; i < plsize; i++) {
            PointType added_pt;
            added_pt.normal_x = 0;
            added_pt.normal_y = 0;
            added_pt.normal_z = 0;
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].reflectivity;
            added_pt.curvature = pl_orig.points[i].t / 1e6;  //ns to ms

            if(added_pt.curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << added_pt.curvature/1000 << RESET << endl;
                continue;
            }

            if (pl_orig.points[i].ring < N_SCANS) {
                if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > blind * blind)
                    pl_surf.points.push_back(added_pt);
            }
        }
    } else {
        pcl::PointCloud<pandar_ros::Point> pl_orig;
        pcl::fromROSMsg(*msg, pl_orig);
        int plsize = pl_orig.points.size();
        pl_surf.reserve(plsize);
        for (int i = 0; i < plsize; i++) {
            PointType added_pt;
            added_pt.normal_x = 0;
            added_pt.normal_y = 0;
            added_pt.normal_z = 0;
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature = (pl_orig.points[i].timestamp - msg->header.stamp.toSec()) * 1000;  //s to ms

            if(added_pt.curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << added_pt.curvature/1000 << RESET << endl;
                continue;
            }

            if (pl_orig.points[i].ring < N_SCANS) {
                if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > blind * blind)
                    pl_surf.points.push_back(added_pt);
            }
        }
    }


    sort(pl_surf.points.begin(), pl_surf.points.end(), time_list_cut_frame);

    //ms
    double last_frame_end_time = msg->header.stamp.toSec() * 1000;
    uint valid_num = 0;
    uint cut_num = 0;
    uint valid_pcl_size = pl_surf.points.size();

    int required_cut_num = required_frame_num;

    if (scan_count < 10)
        required_cut_num = 1;


    PointCloudXYZI pcl_cut;
    for (uint i = 1; i < valid_pcl_size; i++) {
        valid_num++;
        pl_surf[i].curvature += msg->header.stamp.toSec() * 1000 - last_frame_end_time;
        pcl_cut.push_back(pl_surf[i]);

        if (valid_num == (int((cut_num + 1) * valid_pcl_size / required_cut_num) - 1)) {
            cut_num++;
            time_lidar.push_back(last_frame_end_time);
            PointCloudXYZI::Ptr pcl_temp(new PointCloudXYZI()); //shared_ptr一定要初始化
            *pcl_temp = pcl_cut;
            pcl_out.push_back(pcl_temp);
            last_frame_end_time += pl_surf[i].curvature;
            pcl_cut.clear();
            pcl_cut.reserve(valid_pcl_size * 2 / required_frame_num);
        }
    }
}

/**
 * @brief 累积处理Livox激光雷达帧数据（用于多帧累积）
 * @param msg Livox激光雷达消息
 * @param pcl_out 累积输出的点云
 * @param first_lidar_head_time 第一帧激光雷达的起始时间
 */
void Preprocess::process_accumulate_frame_livox(const livox_ros_driver::CustomMsg::ConstPtr &msg,
                                                PointCloudXYZI::Ptr &pcl_out, const double &first_lidar_head_time) {
    int plsize = msg->point_num;
    pl_surf.clear();
    pl_surf.reserve(plsize);
    pl_full.clear();
    pl_full.resize(plsize);

    // 遍历所有点进行预处理
    for (uint i = 1; i < plsize; i++) {
        // 仅保留第0个回波（单次反射），过滤多次反射点
        if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10) ||
            ((msg->points[i].tag & 0x30) == 0x00)) //保留第0个回波
        {
            pl_full[i].x = msg->points[i].x;
            pl_full[i].y = msg->points[i].y;
            pl_full[i].z = msg->points[i].z;
            pl_full[i].intensity = msg->points[i].reflectivity;
            // 使用curvature字段存储每个点的时间（相对于第一帧起始时间），单位：秒
            pl_full[i].curvature =
                    msg->points[i].offset_time / float(1e6) + msg->header.stamp.toSec() - first_lidar_head_time;

            if(msg->points[i].offset_time * 1e-9 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << msg->points[i].offset_time * 1e-9 << RESET << endl;
                continue;
            }

            double dist = pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z;
            if (dist < blind * blind || dist > DET_RANGE * DET_RANGE) continue;

            if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7)
                || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7)
                || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) {
                pl_surf.push_back(pl_full[i]);
            }
        }
    }
    //对点云按时间进行排序
    sort(pl_surf.points.begin(), pl_surf.points.end(), time_list_cut_frame);
    uint valid_pcl_size = pl_surf.points.size();

    for (uint i = 1; i < valid_pcl_size; i++) {
        PointType pt_temp;
        pt_temp = pl_surf[i];
        pcl_out->push_back(pt_temp);
    }
}

/**
 * @brief 处理PointCloud2格式的激光雷达数据（根据类型分发）
 * @param msg PointCloud2格式的激光雷达消息
 * @param pcl_out 输出的处理后点云
 */
void Preprocess::process(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out) {
    // 根据激光雷达类型调用相应的处理函数
    switch (lidar_type) {
        case OUSTER:
            oust_handler(msg);
            break;

        case VELO:
            velodyne_handler(msg);
            break;

        case L515:
            l515_handler(msg);
            break;

        case SIM:
            sim_handler(msg);
            break;

        default:
            printf("Error LiDAR Type");
            break;
    }
    *pcl_out = pl_surf;
}

/**
 * @brief 处理Livox Avia激光雷达数据
 * @param msg Livox激光雷达消息
 *
 * 功能：
 * 1. 如果启用特征提取，则提取平面点和边缘点
 * 2. 如果未启用特征提取，则仅进行基本滤波
 */
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();

    int plsize = msg->point_num;

    pl_corn.reserve(plsize);
    pl_surf.reserve(plsize);
    pl_full.resize(plsize);

    // 清空每条线的缓存
    for (int i = 0; i < N_SCANS; i++) {
        pl_buff[i].clear();
        pl_buff[i].reserve(plsize);
    }

    // 特征提取模式
    if (feature_enabled) {
        for (uint i = 1; i < plsize; i++) {
            if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10) ||
                (msg->points[i].tag & 0x30) == 0x00) {
                pl_full[i].x = msg->points[i].x;
                pl_full[i].y = msg->points[i].y;
                pl_full[i].z = msg->points[i].z;
                pl_full[i].intensity = msg->points[i].reflectivity;
                pl_full[i].curvature =
                        msg->points[i].offset_time / float(1e6); //use curvature as time of each laser points

                if(pl_full[i].curvature/1000 > 1.0/original_freq + 0.02){
                    cout << BOLDRED << "offset time: " << pl_full[i].curvature/1000 << RESET << endl;
                    continue;
                }

                double dist = pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z;
                if (dist < blind * blind) continue;

                if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7)
                    || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7)
                    || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) {
                    pl_buff[msg->points[i].line].push_back(pl_full[i]);
                }
            }
        }
        static int count = 0;
        count++;
        for (int j = 0; j < N_SCANS; j++) {
            if (pl_buff[j].size() <= 5) continue;
            pcl::PointCloud<PointType> &pl = pl_buff[j];
            plsize = pl.size();
            vector<orgtype> &types = typess[j];
            types.clear();
            types.resize(plsize);
            plsize--;
            for (uint i = 0; i < plsize; i++) {
                types[i].range = pl[i].x * pl[i].x + pl[i].y * pl[i].y;
                vx = pl[i].x - pl[i + 1].x;
                vy = pl[i].y - pl[i + 1].y;
                vz = pl[i].z - pl[i + 1].z;
                types[i].dista = vx * vx + vy * vy + vz * vz;
            }
            types[plsize].range = pl[plsize].x * pl[plsize].x + pl[plsize].y * pl[plsize].y;
            give_feature(pl, types);
            // pl_surf += pl;
        }
    } else {
        for (uint i = 1; i < plsize; i++) {
            if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10) ||
                (msg->points[i].tag & 0x30) == 0x00) //保留第0个回波
            {
                pl_full[i].x = msg->points[i].x;
                pl_full[i].y = msg->points[i].y;
                pl_full[i].z = msg->points[i].z;
                pl_full[i].intensity = msg->points[i].reflectivity;
                //use curvature as time of each laser points，unit: ms
                pl_full[i].curvature = msg->points[i].offset_time / float(1e6);
                if(pl_full[i].curvature/1000 > 1.0/original_freq + 0.02){
                    cout << BOLDRED << "offset time: " << pl_full[i].curvature/1000 << RESET << endl;
                    continue;
                }

                double dist = pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z;
                if (dist < blind * blind || dist > DET_RANGE * DET_RANGE) continue;

                if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7)
                    || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7)
                    || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) {
                    pl_surf.push_back(pl_full[i]);
                }
            }
        }
    }
}

void Preprocess::l515_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    pl_surf.clear();
    pl_full.clear();
    pcl::PointCloud<pcl::PointXYZRGB> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    int plsize = pl_orig.size();
    pl_surf.reserve(plsize);

    for (int i = 0; i < pl_orig.points.size(); i++) {

        double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y +
                       pl_orig.points[i].z * pl_orig.points[i].z;

        if (range < blind * blind) continue;

        Eigen::Vector3d pt_vec;
        PointType added_pt;
        added_pt.x = pl_orig.points[i].x;
        added_pt.y = pl_orig.points[i].y;
        added_pt.z = pl_orig.points[i].z;
        added_pt.normal_x = pl_orig.points[i].r;
        added_pt.normal_y = pl_orig.points[i].g;
        added_pt.normal_z = pl_orig.points[i].b;
        added_pt.curvature = 0.0;
        pl_surf.points.push_back(added_pt);
    }
}

void Preprocess::sim_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    pl_surf.clear();
    pl_full.clear();
    pcl::PointCloud<pcl::PointXYZI> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    int plsize = pl_orig.size();
    pl_surf.reserve(plsize);
    for (int i = 0; i < pl_orig.points.size(); i++) {
        double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y +
                       pl_orig.points[i].z * pl_orig.points[i].z;
        if (range < blind * blind || range > DET_RANGE * DET_RANGE) continue;
        Eigen::Vector3d pt_vec;
        PointType added_pt;
        added_pt.x = pl_orig.points[i].x;
        added_pt.y = pl_orig.points[i].y;
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
        added_pt.normal_x = 0;
        added_pt.normal_y = 0;
        added_pt.normal_z = 0;
        added_pt.curvature = 0.0;
        pl_surf.points.push_back(added_pt);
    }
}

void Preprocess::oust_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
    pcl::PointCloud<ouster_ros::Point> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    int plsize = pl_orig.size();
    pl_corn.reserve(plsize);
    pl_surf.reserve(plsize);
    if (feature_enabled) {
        for (int i = 0; i < N_SCANS; i++) {
            pl_buff[i].clear();
            pl_buff[i].reserve(plsize);
        }

        for (uint i = 0; i < plsize; i++) {
            double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y +
                           pl_orig.points[i].z * pl_orig.points[i].z;
            if (range < blind * blind) continue;
            Eigen::Vector3d pt_vec;
            PointType added_pt;
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.normal_x = 0;
            added_pt.normal_y = 0;
            added_pt.normal_z = 0;
            double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.3;
            if (yaw_angle >= 180.0)
                yaw_angle -= 360.0;
            if (yaw_angle <= -180.0)
                yaw_angle += 360.0;

            added_pt.curvature = pl_orig.points[i].t / 1e6;

            if(added_pt.curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << added_pt.curvature/1000 << RESET << endl;
                continue;
            }


            if (pl_orig.points[i].ring < N_SCANS) {
                pl_buff[pl_orig.points[i].ring].push_back(added_pt);
            }
        }

        for (int j = 0; j < N_SCANS; j++) {
            PointCloudXYZI &pl = pl_buff[j];
            int linesize = pl.size();
            vector<orgtype> &types = typess[j];
            types.clear();
            types.resize(linesize);
            linesize--;
            for (uint i = 0; i < linesize; i++) {
                types[i].range = sqrt(pl[i].x * pl[i].x + pl[i].y * pl[i].y);
                vx = pl[i].x - pl[i + 1].x;
                vy = pl[i].y - pl[i + 1].y;
                vz = pl[i].z - pl[i + 1].z;
                types[i].dista = vx * vx + vy * vy + vz * vz;
            }
            types[linesize].range = sqrt(pl[linesize].x * pl[linesize].x + pl[linesize].y * pl[linesize].y);
            give_feature(pl, types);
        }
    } else {
        for (int i = 0; i < pl_orig.points.size(); i++) {
            double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y +
                           pl_orig.points[i].z * pl_orig.points[i].z;

            if (range < blind * blind) continue;

            Eigen::Vector3d pt_vec;
            PointType added_pt;
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].reflectivity;
            added_pt.normal_x = 0;
            added_pt.normal_y = 0;
            added_pt.normal_z = 0;
            added_pt.curvature = pl_orig.points[i].t / 1e6;  //ms
            if(added_pt.curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << added_pt.curvature/1000 << RESET << endl;
                continue;
            }
            if (pl_orig.points[i].ring < N_SCANS)
                pl_surf.points.push_back(added_pt);
        }
    }
}

#define MAX_LINE_NUM 64

void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();

    pcl::PointCloud<velodyne_ros::Point> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    int plsize = pl_orig.points.size();
    pl_surf.reserve(plsize);

    bool is_first[MAX_LINE_NUM];
    double yaw_fp[MAX_LINE_NUM] = {0};     // yaw of first scan point
    double omega_l = 3.61;       // scan angular velocity (deg/ms)
    float yaw_last[MAX_LINE_NUM] = {0.0};  // yaw of last scan point
    float time_last[MAX_LINE_NUM] = {0.0}; // last offset time

    if (pl_orig.points[plsize - 1].time > 0) {
        cout << "Use given offset time." << endl;
        given_offset_time = true;
    } else {
        cout << "Compute offset time using constant rotation model." << endl;
        given_offset_time = false;
        memset(is_first, true, sizeof(is_first));
        double yaw_first = atan2(pl_orig.points[0].y, pl_orig.points[0].x) * 57.29578;
        double yaw_end = yaw_first;
        int layer_first = pl_orig.points[0].ring;
        for (uint i = plsize - 1; i > 0; i--) {
            if (pl_orig.points[i].ring == layer_first) {
                yaw_end = atan2(pl_orig.points[i].y, pl_orig.points[i].x) * 57.29578;
                break;
            }
        }
    }

    if (feature_enabled) {
        for (int i = 0; i < N_SCANS; i++) {
            pl_buff[i].clear();
            pl_buff[i].reserve(plsize);
        }

        for (int i = 0; i < plsize; i++) {
            PointType added_pt;
            added_pt.normal_x = 0;
            added_pt.normal_y = 0;
            added_pt.normal_z = 0;
            int layer = pl_orig.points[i].ring;
            if (layer >= N_SCANS) continue;
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature = pl_orig.points[i].time * 1000.0; // unit: ms

            if(added_pt.curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << added_pt.curvature/1000 << RESET << endl;
                continue;
            }


            if (!given_offset_time) {
                double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.2957;
                if (is_first[layer]) {
                    // printf("layer: %d; is first: %d", layer, is_first[layer]);
                    yaw_fp[layer] = yaw_angle;
                    is_first[layer] = false;
                    added_pt.curvature = 0.0;
                    yaw_last[layer] = yaw_angle;
                    time_last[layer] = added_pt.curvature;
                    continue;
                }

                if (yaw_angle <= yaw_fp[layer]) {
                    added_pt.curvature = (yaw_fp[layer] - yaw_angle) / omega_l;
                } else {
                    added_pt.curvature = (yaw_fp[layer] - yaw_angle + 360.0) / omega_l;
                }

                if (added_pt.curvature < time_last[layer]) added_pt.curvature += 360.0 / omega_l;

                yaw_last[layer] = yaw_angle;
                time_last[layer] = added_pt.curvature;
            }

            pl_buff[layer].points.push_back(added_pt);
        }

        for (int j = 0; j < N_SCANS; j++) {
            PointCloudXYZI &pl = pl_buff[j];
            int linesize = pl.size();
            if (linesize < 2) continue;
            vector<orgtype> &types = typess[j];
            types.clear();
            types.resize(linesize);
            linesize--;
            for (uint i = 0; i < linesize; i++) {
                types[i].range = sqrt(pl[i].x * pl[i].x + pl[i].y * pl[i].y);
                vx = pl[i].x - pl[i + 1].x;
                vy = pl[i].y - pl[i + 1].y;
                vz = pl[i].z - pl[i + 1].z;
                types[i].dista = vx * vx + vy * vy + vz * vz;
            }
            types[linesize].range = sqrt(pl[linesize].x * pl[linesize].x + pl[linesize].y * pl[linesize].y);
            give_feature(pl, types);
        }
    } else {
        for (int i = 0; i < plsize; i++) {
            PointType added_pt;

            added_pt.normal_x = 0;
            added_pt.normal_y = 0;
            added_pt.normal_z = 0;
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature = pl_orig.points[i].time * 1000.0; //ms

            if(added_pt.curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << added_pt.curvature/1000 << RESET << endl;
                continue;
            }


            if (!given_offset_time) {
                int layer = pl_orig.points[i].ring;
                double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.2957;

                if (is_first[layer]) {
                    yaw_fp[layer] = yaw_angle;
                    is_first[layer] = false;
                    added_pt.curvature = 0.0;
                    yaw_last[layer] = yaw_angle;
                    time_last[layer] = added_pt.curvature;
                    continue;
                }

                // compute offset time
                if (yaw_angle <= yaw_fp[layer]) {
                    added_pt.curvature = (yaw_fp[layer] - yaw_angle) / omega_l;
                } else {
                    added_pt.curvature = (yaw_fp[layer] - yaw_angle + 360.0) / omega_l;
                }

                if (added_pt.curvature < time_last[layer]) added_pt.curvature += 360.0 / omega_l;

                yaw_last[layer] = yaw_angle;
                time_last[layer] = added_pt.curvature;
            }


            if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > blind * blind) {
                pl_surf.points.push_back(added_pt);
            }

        }
    }
}

/**
 * @brief 提取点云特征（平面点和边缘点）
 * @param pl 输入的点云（某一条线）
 * @param types 点云特征类型数组
 *
 * 功能：
 * 1. 检测平面点（Real_Plane, Poss_Plane）
 * 2. 检测边缘点（Edge_Jump, Edge_Plane）
 * 3. 检测线状特征（Wire）
 * 4. 提取并输出特征点到pl_surf和pl_corn
 */
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types) {
    int plsize = pl.size();
    int plsize2;
    if (plsize == 0) {
        printf("something wrong\n");
        return;
    }
    uint head = 0;

    // 跳过盲区内的点
    while (types[head].range < blind) {
        head++;
    }

    // 平面特征提取
    plsize2 = (plsize > group_size) ? (plsize - group_size) : 0;

    Eigen::Vector3d curr_direct(Eigen::Vector3d::Zero());  // 当前平面方向向量
    Eigen::Vector3d last_direct(Eigen::Vector3d::Zero());  // 上一个平面方向向量

    uint i_nex = 0, i2;
    uint last_i = 0;
    uint last_i_nex = 0;
    int last_state = 0;  // 上一次状态：0-非平面，1-平面
    int plane_type;

    // 第一步：平面点检测
    for (uint i = head; i < plsize2; i++) {
        if (types[i].range < blind) {
            continue;
        }

        i2 = i;

        // 判断当前点及其后续点是否构成平面
        plane_type = plane_judge(pl, types, i, i_nex, curr_direct);

        if (plane_type == 1) {
            // 标记平面上的点
            for (uint j = i; j <= i_nex; j++) {
                if (j != i && j != i_nex) {
                    types[j].ftype = Real_Plane;  // 平面内部点
                } else {
                    types[j].ftype = Poss_Plane;  // 平面边界点
                }
            }

            // 检测两个平面的交界处（边缘平面）
            if (last_state == 1 && last_direct.norm() > 0.1) {
                double mod = last_direct.transpose() * curr_direct;
                // 如果两个平面法向量夹角较大（45-135度），则为边缘
                if (mod > -0.707 && mod < 0.707) {
                    types[i].ftype = Edge_Plane;
                } else {
                    types[i].ftype = Real_Plane;
                }
            }

            i = i_nex - 1;
            last_state = 1;
        } else // 非平面
        {
            i = i_nex;
            last_state = 0;
        }
        // else if(plane_type == 0)
        // {
        //   if(last_state == 1)
        //   {
        //     uint i_nex_tem;
        //     uint j;
        //     for(j=last_i+1; j<=last_i_nex; j++)
        //     {
        //       uint i_nex_tem2 = i_nex_tem;
        //       Eigen::Vector3d curr_direct2;

        //       uint ttem = plane_judge(pl, types, j, i_nex_tem, curr_direct2);

        //       if(ttem != 1)
        //       {
        //         i_nex_tem = i_nex_tem2;
        //         break;
        //       }
        //       curr_direct = curr_direct2;
        //     }

        //     if(j == last_i+1)
        //     {
        //       last_state = 0;
        //     }
        //     else
        //     {
        //       for(uint k=last_i_nex; k<=i_nex_tem; k++)
        //       {
        //         if(k != i_nex_tem)
        //         {
        //           types[k].ftype = Real_Plane;
        //         }
        //         else
        //         {
        //           types[k].ftype = Poss_Plane;
        //         }
        //       }
        //       i = i_nex_tem-1;
        //       i_nex = i_nex_tem;
        //       i2 = j-1;
        //       last_state = 1;
        //     }

        //   }
        // }

        last_i = i2;
        last_i_nex = i_nex;
        last_direct = curr_direct;
    }

    // 第二步：边缘跳跃点检测
    plsize2 = plsize > 3 ? plsize - 3 : 0;
    for (uint i = head + 3; i < plsize2; i++) {
        // 跳过盲区点和已标记为平面的点
        if (types[i].range < blind || types[i].ftype >= Real_Plane) {
            continue;
        }

        if (types[i - 1].dista < 1e-16 || types[i].dista < 1e-16) {
            continue;
        }

        Eigen::Vector3d vec_a(pl[i].x, pl[i].y, pl[i].z);
        Eigen::Vector3d vecs[2];

        // 计算当前点与前后点的夹角
        for (int j = 0; j < 2; j++) {
            int m = -1;
            if (j == 1) {
                m = 1;
            }

            // 检查前后点是否在盲区
            if (types[i + m].range < blind) {
                if (types[i].range > inf_bound) {
                    types[i].edj[j] = Nr_inf;    // 无穷远跳跃
                } else {
                    types[i].edj[j] = Nr_blind;  // 盲区跳跃
                }
                continue;
            }

            vecs[j] = Eigen::Vector3d(pl[i + m].x, pl[i + m].y, pl[i + m].z);
            vecs[j] = vecs[j] - vec_a;

            // 计算夹角余弦值
            types[i].angle[j] = vec_a.dot(vecs[j]) / vec_a.norm() / vecs[j].norm();
            if (types[i].angle[j] < jump_up_limit) {
                types[i].edj[j] = Nr_180;  // 接近180度跳跃
            } else if (types[i].angle[j] > jump_down_limit) {
                types[i].edj[j] = Nr_zero; // 接近0度跳跃
            }
        }

        // 计算前后向量的交角
        types[i].intersect = vecs[Prev].dot(vecs[Next]) / vecs[Prev].norm() / vecs[Next].norm();

        // 检测边缘跳跃的各种情况
        if (types[i].edj[Prev] == Nr_nor && types[i].edj[Next] == Nr_zero && types[i].dista > 0.0225 &&
            types[i].dista > 4 * types[i - 1].dista) {
            // 前面正常，后面跳跃，且距离突变
            if (types[i].intersect > cos160) {
                if (edge_jump_judge(pl, types, i, Prev)) {
                    types[i].ftype = Edge_Jump;
                }
            }
        } else if (types[i].edj[Prev] == Nr_zero && types[i].edj[Next] == Nr_nor && types[i - 1].dista > 0.0225 &&
                   types[i - 1].dista > 4 * types[i].dista) {
            // 前面跳跃，后面正常，且距离突变
            if (types[i].intersect > cos160) {
                if (edge_jump_judge(pl, types, i, Next)) {
                    types[i].ftype = Edge_Jump;
                }
            }
        } else if (types[i].edj[Prev] == Nr_nor && types[i].edj[Next] == Nr_inf) {
            // 后面点在无穷远
            if (edge_jump_judge(pl, types, i, Prev)) {
                types[i].ftype = Edge_Jump;
            }
        } else if (types[i].edj[Prev] == Nr_inf && types[i].edj[Next] == Nr_nor) {
            // 前面点在无穷远
            if (edge_jump_judge(pl, types, i, Next)) {
                types[i].ftype = Edge_Jump;
            }

        } else if (types[i].edj[Prev] > Nr_nor && types[i].edj[Next] > Nr_nor) {
            // 前后都有跳跃，可能是线状特征（如电线）
            if (types[i].ftype == Nor) {
                types[i].ftype = Wire;
            }
        }
    }

    // 第三步：小平面检测（基于距离比和交角）
    plsize2 = plsize - 1;
    double ratio;
    for (uint i = head + 1; i < plsize2; i++) {
        // 跳过盲区点
        if (types[i].range < blind || types[i - 1].range < blind || types[i + 1].range < blind) {
            continue;
        }

        if (types[i - 1].dista < 1e-8 || types[i].dista < 1e-8) {
            continue;
        }

        if (types[i].ftype == Nor) {
            // 计算前后距离比
            if (types[i - 1].dista > types[i].dista) {
                ratio = types[i - 1].dista / types[i].dista;
            } else {
                ratio = types[i].dista / types[i - 1].dista;
            }

            // 如果交角小且距离比小，则认为是平面
            if (types[i].intersect < smallp_intersect && ratio < smallp_ratio) {
                if (types[i - 1].ftype == Nor) {
                    types[i - 1].ftype = Real_Plane;
                }
                if (types[i + 1].ftype == Nor) {
                    types[i + 1].ftype = Real_Plane;
                }
                types[i].ftype = Real_Plane;
            }
        }
    }

    // 第四步：提取特征点并进行降采样
    int last_surface = -1;
    for (uint j = head; j < plsize; j++) {
        // 处理平面点
        if (types[j].ftype == Poss_Plane || types[j].ftype == Real_Plane) {
            if (last_surface == -1) {
                last_surface = j;
            }

            // 每隔point_filter_num个点采样一个平面点
            if (j == uint(last_surface + point_filter_num - 1)) {
                PointType ap;
                ap.x = pl[j].x;
                ap.y = pl[j].y;
                ap.z = pl[j].z;
                ap.curvature = pl[j].curvature;
                pl_surf.push_back(ap);

                last_surface = -1;
            }
        } else {
            // 处理边缘点
            if (types[j].ftype == Edge_Jump || types[j].ftype == Edge_Plane) {
                pl_corn.push_back(pl[j]);
            }
            // 对剩余的平面点求平均
            if (last_surface != -1) {
                PointType ap;
                for (uint k = last_surface; k < j; k++) {
                    ap.x += pl[k].x;
                    ap.y += pl[k].y;
                    ap.z += pl[k].z;
                    ap.curvature += pl[k].curvature;
                }
                ap.x /= (j - last_surface);
                ap.y /= (j - last_surface);
                ap.z /= (j - last_surface);
                ap.curvature /= (j - last_surface);
                pl_surf.push_back(ap);
            }
            last_surface = -1;
        }
    }
}


/**
 * @brief 判断点群是否构成平面
 * @param pl 输入点云
 * @param types 点特征类型数组
 * @param i_cur 当前点索引
 * @param i_nex 输出：平面的结束点索引
 * @param curr_direct 输出：平面方向向量
 * @return 0-非平面，1-平面，2-遇到盲区
 */
int Preprocess::plane_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i_cur, uint &i_nex,
                            Eigen::Vector3d &curr_direct) {
    // 计算点群的距离阈值（随距离自适应）
    double group_dis = disA * types[i_cur].range + disB;
    group_dis = group_dis * group_dis;

    double two_dis;
    vector<double> disarr;  // 存储点间距离
    disarr.reserve(20);

    // 先收集group_size个点
    for (i_nex = i_cur; i_nex < i_cur + group_size; i_nex++) {
        if (types[i_nex].range < blind) {
            curr_direct.setZero();
            return 2;  // 遇到盲区
        }
        disarr.push_back(types[i_nex].dista);
    }

    // 继续向后扩展，直到距离超过阈值
    for (;;) {
        if ((i_cur >= pl.size()) || (i_nex >= pl.size())) break;

        if (types[i_nex].range < blind) {
            curr_direct.setZero();
            return 2;  // 遇到盲区
        }
        // 计算当前点与起始点的距离
        vx = pl[i_nex].x - pl[i_cur].x;
        vy = pl[i_nex].y - pl[i_cur].y;
        vz = pl[i_nex].z - pl[i_cur].z;
        two_dis = vx * vx + vy * vy + vz * vz;
        if (two_dis >= group_dis) {
            break;  // 距离超过阈值，停止扩展
        }
        disarr.push_back(types[i_nex].dista);
        i_nex++;
    }

    // 计算点群的"宽度"（点到主方向线的最大距离）
    double leng_wid = 0;
    double v1[3], v2[3];
    for (uint j = i_cur + 1; j < i_nex; j++) {
        if ((j >= pl.size()) || (i_cur >= pl.size())) break;
        // v1: 从起始点到当前点的向量
        v1[0] = pl[j].x - pl[i_cur].x;
        v1[1] = pl[j].y - pl[i_cur].y;
        v1[2] = pl[j].z - pl[i_cur].z;

        // v2: v1与主方向向量(vx,vy,vz)的叉乘，得到点到线的距离向量
        v2[0] = v1[1] * vz - vy * v1[2];
        v2[1] = v1[2] * vx - v1[0] * vz;
        v2[2] = v1[0] * vy - vx * v1[1];

        double lw = v2[0] * v2[0] + v2[1] * v2[1] + v2[2] * v2[2];
        if (lw > leng_wid) {
            leng_wid = lw;
        }
    }

    // 判断点群的"细长度"：如果点群过于扁平，则不是平面
    if ((two_dis * two_dis / leng_wid) < p2l_ratio) {
        curr_direct.setZero();
        return 0;  // 非平面
    }

    // 对点间距离进行降序排序
    uint disarrsize = disarr.size();
    for (uint j = 0; j < disarrsize - 1; j++) {
        for (uint k = j + 1; k < disarrsize; k++) {
            if (disarr[j] < disarr[k]) {
                leng_wid = disarr[j];
                disarr[j] = disarr[k];
                disarr[k] = leng_wid;
            }
        }
    }

    // 检查最小距离是否过小
    if (disarr[disarr.size() - 2] < 1e-16) {
        curr_direct.setZero();
        return 0;
    }

    // 根据激光雷达类型选择不同的平面判断标准
    if (lidar_type == AVIA) {
        // Livox：检查最大/中间和中间/最小距离比
        double dismax_mid = disarr[0] / disarr[disarrsize / 2];
        double dismid_min = disarr[disarrsize / 2] / disarr[disarrsize - 2];

        if (dismax_mid >= limit_maxmid || dismid_min >= limit_midmin) {
            curr_direct.setZero();
            return 0;  // 距离变化过大，非平面
        }
    } else {
        // 其他激光雷达：检查最大/最小距离比
        double dismax_min = disarr[0] / disarr[disarrsize - 2];
        if (dismax_min >= limit_maxmin) {
            curr_direct.setZero();
            return 0;  // 距离变化过大，非平面
        }
    }

    // 输出平面方向向量并归一化
    curr_direct << vx, vy, vz;
    curr_direct.normalize();
    return 1;  // 是平面
}

/**
 * @brief 判断是否为边缘跳跃点（通过检查距离突变）
 * @param pl 输入点云
 * @param types 点特征类型数组
 * @param i 当前点索引
 * @param nor_dir 检测方向（Prev=0向前，Next=1向后）
 * @return true-是边缘跳跃点，false-不是
 */
bool Preprocess::edge_jump_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i, Surround nor_dir) {
    // 检查相邻点是否在盲区
    if (nor_dir == 0) {
        // 向前检测
        if (types[i - 1].range < blind || types[i - 2].range < blind) {
            return false;
        }
    } else if (nor_dir == 1) {
        // 向后检测
        if (types[i + 1].range < blind || types[i + 2].range < blind) {
            return false;
        }
    }

    // 获取两个相邻点的距离
    double d1 = types[i + nor_dir - 1].dista;
    double d2 = types[i + 3 * nor_dir - 2].dista;
    double d;

    // 确保d1 >= d2
    if (d1 < d2) {
        d = d1;
        d1 = d2;
        d2 = d;
    }

    d1 = sqrt(d1);
    d2 = sqrt(d2);

    // 判断距离比和距离差是否超过阈值
    // 如果距离变化过大，则不是边缘跳跃（可能是噪声）
    if (d1 > edgea * d2 || (d1 - d2) > edgeb) {
        return false;
    }

    return true;
}