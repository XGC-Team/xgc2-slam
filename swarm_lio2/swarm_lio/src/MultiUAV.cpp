 ...
 * @brief 多无人机系统构造函数
 * @param nh ROS节点句柄
 * @param drone_id_ 本机无人机ID
 * 初始化ROS通信、加载参数、设置订阅器和发布器
 */
Multi_UAV::Multi_UAV(const ros::NodeHandle &nh, const int & drone_id_) {
    ...
    // === 设置默认阈值参数 ===
    same_obj_thresh = 0.3; // 判定同一物体的距离阈值
    valid_temp_cluster_dist_thresh = 0.4; // 临时跟踪器有效聚类距离阈值
    topic_name_prefix = ""; // 话题前缀（默认为空）
    text_scale = 0.3; // 文本可视化缩放
    mesh_scale = 0.8; // 网格模型缩放
    ...
    // === 初始化数据结构 ===
    temp_tracker.clear(); // 清空临时跟踪器
    teammate_tracker.clear(); // 清空队友跟踪器
    reconnected_id.clear(); // 清空重连ID列表
    extrinsic_infection.drone_id = drone_id; // 设置外参传染模型的无人机ID
    degenerated = false; // 初始化退化标志
    teammate_id_by_traj_matching.clear(); // 清空轨迹匹配识别的队友ID列表
}
 ...
 */
Multi_UAV::~Multi_UAV(){
    teammate_tracker.clear(); // 清空队友跟踪器映射
    temp_tracker.clear(); // 清空临时跟踪器
    teammates.clear(); // 清空队友信息映射
    cluster_pos_tag.clear(); // 清空聚类结果
    reconnected_id.clear(); // 清空重连ID列表
}

/**
 * @brief 从上三角向量构建协方差矩阵
 * @param vec 12维向量，包含姿态和位置协方差的上三角元素
 * @param rot_cov 输出：姿态协方差矩阵（3x3）
 * @param pos_cov 输出：位置协方差矩阵（3x3）
 *
 * 向量布局：[R00, R01, R02, R11, R12, R22, P00, P01, P02, P11, P12, P22]
 */
...
/**
 * @brief 队友状态消息回调函数
 * @param msg 接收到的队友状态消息
 *
 * 处理来自其他无人机的状态信息，包括位姿、速度、协方差和互观测信息
 * 检测队友是否重连（通过启动时间变化判断）
 */
...
/**
 * @brief 全局外参消息回调函数
 * @param msg 接收到的全局外参状态消息
 *
 * 接收其他无人机广播的全局外参信息，构建外参传染图
 * 需要等待2秒后才开始接收，以确保重连的队友外参已被清除
 */
void Multi_UAV::GlobalExtrinsicCbk(const swarm_msgs::GlobalExtrinsicStatus::ConstPtr &msg) {
    // 如果已找到所有队友或收到自己的消息，则忽略
    ...
    // 与队友建立通讯后等待2秒再开始接收外参信息
    // 这2秒内其他飞机会通过ResetReconnectedGlobalExtrinsic()删除重连队友的旧外参
    ...
    if(iter == teammates.end())
        ...
        if(duration < 2.0 || iter->second.first_connect_time < 0)
            return; // 连接时间不足2秒，暂不接收外参
    }

    // 遍历消息中所有的外参关系（j号无人机与其他k号无人机的外参）
    for (int k = 0; k < msg->extrinsic.size(); ++k) {
        int id_k = msg->extrinsic[k].teammate_id; // k号无人机ID

        // 提取旋转（欧拉角，度 -> 弧度）
        V3D rot_jk_rad(msg->extrinsic[k].rot_deg[0], msg->extrinsic[k].rot_deg[1],
                       msg->extrinsic[k].rot_deg[2]);
        rot_jk_rad /= 57.3; // 角度转弧度
        M3D rot_jk = EulerToRotM(rot_jk_rad); // 转换为旋转矩阵
        ...
        edge.from = id_j;   // 起始节点
        edge.to = id_k;     // 终止节点
        edge.rotation = rot_jk;     // j到k的旋转
        edge.translation = trans_jk; // j到k的平移
...
/**
 * @brief 重置重连队友的全局外参
 * @param state_in 状态输入（将被修改）
 * @param lidar_end_time 激光雷达帧结束时刻
 *
 * 当检测到队友重启时，清除其旧的跟踪器和全局外参信息
 */
void Multi_UAV::ResetReconnectedGlobalExtrinsic(StatesGroup &state_in, const double &lidar_end_time){
    ...
    for (int i = 0; i < reconnected_id.size(); ++i) {
        ...
        if (tracker_ != teammate_tracker.end()) {
            ...
            found_all_teammates = false; // 需要重新寻找所有队友
        }
...
/**
 * @brief 更新因子图：使用ISAM2优化全局外参
 * @param print_log 是否打印日志
 *
 * 基于外参传染模型，通过因子图优化计算所有队友的全局外参
 */
...
void Multi_UAV::UpdateGlobalExtrinsicAndCreateNewTeammateTracker(StatesGroup &state_in, const double &lidar_end_time) {
    ...
    for (auto iter = teammates.begin(); iter != teammates.end(); ++iter) {
        ...
        if (state_in.global_extrinsic_trans[id].norm() < 0.001 && state.global_extrinsic_trans[id].norm() > 0.001) {
            ...
            // 创建新的EKF跟踪器
            ESIKF drone_tracker(6, 3, 6); // 6维状态（位置+速度），3维观测，6维噪声
            ...
            V3D teammate_vel_in_world =
                    state.global_extrinsic_rot[id] * teammate.teammate_state.vel;

            // 初始化EKF状态
            Matrix<double, 6, 1> ekf_init_state;
            ekf_init_state.head(3) = teammate_pos_in_world; // 位置
            ekf_init_state.tail(3) = teammate_vel_in_world; // 速度
...
/**
 * @brief 将临时缓存的队友状态复制到正式状态
 * @param teammate 队友信息结构
 */
...
void Multi_UAV::ResetTeammateState() {
    for (auto iter = teammates.begin(); iter != teammates.end(); ++iter) {
        iter->second.teammate_state_temp.is_observed = false;  // 重置"被队友观测"标志
        iter->second.teammate_state_temp.my_pos_in_teammate = Zero3d;
        iter->second.teammate_state_temp.degenerated = false;
        iter->second.is_observe_teammate = false; // 重置"观测到队友"标志
...
/**
 * @brief 发布本机状态消息
 * @param unbiased_gyr 去偏后的角速度
 * @param lidar_end_time 激光雷达帧结束时刻
 * @param first_lidar_time 首帧激光雷达时刻（系统启动时间）
 *
 * 向其他无人机广播本机的位姿、速度、协方差和对队友的观测信息
 */
void Multi_UAV::PublishQuadstate(const V3D &unbiased_gyr, const double &lidar_end_time, const double &first_lidar_time) {
    quadstate_msg_pub.teammate.clear(); // 清空队友观测列表
    ...
    SetPosestamp(quadstate_msg_pub.pose); // 设置位姿

    // 转换世界系到重力系的旋转角度（弧度转角度）
    ...
    // 提取位姿协方差矩阵的上三角元素
    // 格式：[R00, R01, R02, R11, R12, R22, P00, P01, P02, P11, P12, P22]
    ...
    quadstate_msg_pub.swarmlio_start_time = first_lidar_time; // 系统启动时间
    quadstate_msg_pub.degenerated = degenerated; // 退化状态标志

    // 添加观测到的队友信息
    for (auto iter = teammates.begin(); iter != teammates.end(); ++iter) {
        if (iter->second.is_observe_teammate) { // 如果观测到该队友
            swarm_msgs::ObserveTeammate obs_teammate;
            obs_teammate.is_observe = true;
            obs_teammate.teammate_id = iter->first; // 队友ID
            // 队友在本机体坐标系下的位置
...
/**
 * @brief 在预测区域内进行欧几里得聚类提取
 * @param lidar_end_time 激光雷达帧结束时刻
 * @param cur_pcl_undistort 当前去畸变后的点云
 *
 * 基于队友跟踪器和临时跟踪器的预测位置，提取预测区域内的点云聚类
 */
void Multi_UAV::ClusterExtractPredictRegion(const double &lidar_end_time, const PointCloudXYZI::Ptr cur_pcl_undistort) {
    ...
    cur_pcl_undistort_remove_far_points->clear();
    cur_pcl_undistort_remove_far_points->reserve(cur_pcl_undistort->size());

    // === 移除远距离点（超过20米） ===
#ifdef MP_EN
    omp_set_num_threads(MP_PROC_NUM); // 多线程处理
#pragma omp parallel for
#endif
    for (int i = 0; i < cur_pcl_undistort->size(); i++) {
        auto it_pcl = cur_pcl_undistort->points.begin() + i;
        V3D pt(it_pcl->x, it_pcl->y, it_pcl->z);
        if(pt.norm() < 20) // 只保留20米以内的点
...
/**
 * @brief 轨迹匹配：使用SVD分解计算两条轨迹之间的变换关系
 * @param dyn_positions 本机观测到的动态物体轨迹（在本机世界系下）
 * @param uav_positions 队友广播的轨迹（在队友世界系下）
 * @param Rot 输出：旋转矩阵
 * @param trans 输出：平移向量
 * @param Coeff 输出：匹配质量系数
 * @param id 队友ID
 * @return 是否匹配成功
 *
 * 算法原理：
 * 1. 计算两组轨迹点的质心
 * 2. 去质心化
 * 3. 构建相关矩阵H = Σ(uav_i - uav_mean)(dyn_i - dyn_mean)^T
 * 4. SVD分解H得到旋转矩阵
 * 5. 计算平移向量
 * 6. 评估匹配误差
 */
bool Multi_UAV::TrajMatching(vector<Vector3d> &dyn_positions,
                             ...
                             const int &id) {
    // === 步骤1：计算两组轨迹的质心 ===
    V3D dyn_mean = Zero3d; // 动态物体轨迹质心
    V3D uav_mean = Zero3d; // 队友轨迹质心
    ...
    M3D H = M3D::Zero();
    for (size_t i = 0; i < dyn_positions.size(); ++i) {
        // H = Σ(uav_i - uav_mean)(dyn_i - dyn_mean)^T
...
/**
 * @brief 创建队友跟踪器：通过轨迹匹配确认临时跟踪器为已知队友
 * @param lidar_end_time 激光雷达帧结束时刻
 * @param index 临时跟踪器索引
 * @param state_in 状态输入（将被修改）
 * @param state_prop 状态传播（将被修改）
 * @param print_log 是否打印日志
 * @return 是否成功创建队友跟踪器
 *
 * 流程：
 * 1. 评估临时跟踪器轨迹的激励程度（通过SVD奇异值）
 * 2. 与所有已连接但未跟踪的队友进行轨迹匹配
 * 3. 匹配成功则创建队友跟踪器并删除临时跟踪器
 */
bool Multi_UAV::CreateTeammateTracker(const double &lidar_end_time, const int &index, StatesGroup &state_in,
    ...
    // SVD分解，评估轨迹激励程度
    JacobiSVD<MatrixXd> svd(H, Eigen::ComputeThinU | Eigen::ComputeThinV);
    double second_large_singular_value = svd.singularValues()(1); // 第二大奇异值
    ...
    if (second_large_singular_value > traj_matching_start_thresh) {
        ...
        for (auto iter = teammates.begin(); iter != teammates.end(); ++iter) {
            ...
            if (iter->second.teammate_odom_time.empty() ||
                iter_tracker != teammate_tracker.end())
                continue;
            ...
            // 尝试匹配该队友
                ...
                // 遍历队友的里程计轨迹
                for (int i = 0; i < iter->second.teammate_odom_time.size(); ++i) {
                    // 移除时间戳早于队友轨迹的观测点
