...
#include "IMUProcessing.hpp"                    // IMU数据处理
#include <unistd.h>                             // Unix标准函数库
#include <Python.h>                             // Python接口
#include <Eigen/Core>                           // Eigen核心库
#include <Eigen/Sparse>                         // Eigen稀疏矩阵库
#include <nav_msgs/Odometry.h>                  // ROS里程计消息
#include <nav_msgs/Path.h>                      // ROS路径消息
#include <algorithm>                            // 标准算法库
#include <pcl_conversions/pcl_conversions.h>    // PCL与ROS消息转换
#include <pcl/filters/voxel_grid.h>             // PCL体素滤波器
#include <pcl/io/pcd_io.h>                      // PCD文件读写
#include <tf/transform_datatypes.h>             // TF坐标变换数据类型
#include <tf/transform_broadcaster.h>           // TF坐标变换广播器
#include <livox_ros_driver/CustomMsg.h>         // Livox雷达自定义消息
#include "preprocess.h"                         // 点云预处理
#include <ikd-Tree/ikd_Tree.h>                  // ikd-Tree增量式KD树
#include "MultiUAV.h"                           // 多无人机协同
#include <arpa/inet.h>                          // 网络地址转换
#include <netinet/in.h>                         // 网络协议
#include <ifaddrs.h>                            // 网络接口地址
#include "cpu_memory_query.h"                   // CPU和内存查询
...
// 宏定义和常量
#define LASER_POINT_COV     (0.001)    // 激光点云协方差
#define MAXN                (720000)   // 最大点云数量
float DET_RANGE = 300.0f;              // 检测范围(米)
const float MOV_THRESHOLD = 1.5f;      // 移动阈值

// 多线程同步变量
mutex mtx_buffer_lidar, mtx_buffer_imu, mtx_buffer_imu_prop, mtx_buffer_gt;  // 互斥锁：雷达、IMU、IMU传播、真值
condition_variable sig_buffer;         // 条件变量：缓冲区信号
...
// 地图立方体长度、总距离、雷达结束时间、首帧雷达时间
double cube_len = 0, total_distance = 0, lidar_end_time = 0, first_lidar_time = - 1.0;
ros::Publisher pubDrone;  // 无人机位姿发布器

// 时间日志变量
double ave_pre_processing_time = 0.0, ave_frame_time = 0.0;  // 平均预处理时间、平均帧时间
int kdtree_delete_counter = 0, kdtree_size_st = 0, kdtree_size_end = 0, add_point_size = 0;  // KD树统计变量

// 雷达和PCD保存参数
int lidar_type, pcd_save_interval = -1, pcd_index = 0;  // 雷达类型、PCD保存间隔、PCD索引
bool lidar_pushed, flg_reset, flg_exit = false, flg_EKF_inited = true;  // 雷达数据推送标志、重置标志、退出标志、EKF初始化标志
bool imu_en = true;  // IMU使能标志
bool scan_pub_en = false, dense_pub_en = false, scan_body_pub_en = false;  // 点云发布使能标志
bool runtime_pos_log = false, pcd_save_en = false, path_en = true;  // 运行时位置日志、PCD保存使能、路径使能
double pcd_resolution = 0.03;;  // PCD分辨率

// 雷达-IMU参数
bool cut_frame_en{false}, accumulate_frame_en{false};  // 切帧使能、累积帧使能
int cut_frame_num = 1, original_frequency = 10, frame_num = 0, imu_num = 0;  // 切帧数、原始频率、帧号、IMU数量
vector<double> extrinT(3, 0.0);  // 雷达到IMU外参：平移
vector<double> extrinR(9, 0.0);  // 雷达到IMU外参：旋转
M3D offset_R_L_I(M3D::Identity());  // 雷达到IMU旋转矩阵
V3D offset_T_L_I(Zero3d);           // 雷达到IMU平移向量
double time_offset_lidar_imu = 0.0; // 雷达和IMU时间偏移

// 重力对齐参数
bool gravity_align_en{false}, gravity_align_finished{false};  // 重力对齐使能、重力对齐完成标志
double gravity_align_dur;  // 重力对齐持续时间
Matrix4d G_T_PX4, PX4_T_I, G_T_I0, Horizon_T_PX4;  // 各坐标系之间的变换矩阵
V3D unbiased_gyr;  // 去偏差陀螺仪数据

// 多无人机参数
ofstream fout_pose;              // 位姿输出文件流
string sub_gt_pose_topic;        // 真值位姿话题
deque<VD(7)> ground_truth;       // 真值队列(欧拉角3 + 位置3 + 时间戳1)
deque<VD(4)> LIO_pose;           // LIO位姿队列(位置3 + 时间戳1)
int drone_id, actual_uav_num;    // 无人机ID、实际无人机数量
double mutual_observe_noise, mutual_observe_noise_;  // 互相观测噪声
shared_ptr<ImuProcess> p_imu;    // IMU处理器指针
string topic_name_prefix;        // 话题名称前缀
int frame_num_in_sliding_window; // 滑动窗口中的帧数
double text_scale, mesh_scale;   // 文字缩放、网格缩放
int degeneration_thresh;         // 退化阈值

// IMU传播参数
StatesGroup imu_propagate, latest_ekf_state;  // IMU传播状态、最新EKF状态
bool new_imu{false}, state_update_flg{false}, imu_prop_enable{true}, ekf_finish_once{false};  // IMU相关标志位
...
string imu_prop_topic;                    // IMU传播话题

// 点云保存相关
PointCloudXYZI::Ptr pcl_wait_save(new PointCloudXYZI());          // 待保存点云
PointCloudXYZI::Ptr voxel_downsampled_map(new PointCloudXYZI());  // 体素降采样地图
pcl::PCDWriter pcd_writer;  // PCD文件写入器
string all_points_dir;      // 所有点云保存目录

// 数据缓冲区
vector<BoxPointType> cub_needrm;                 // 需要删除的立方体区域
deque<PointCloudXYZI::Ptr> lidar_buffer;         // 雷达点云缓冲区
deque<double> time_buffer;                       // 时间戳缓冲区
deque<sensor_msgs::Imu::ConstPtr> imu_buffer;    // IMU数据缓冲区
vector<vector<int>> pointSearchInd_surf;         // 面特征点搜索索引
vector<PointVector> Nearest_Points;              // 最近邻点集合
bool point_selected_surf[100000] = {0};          // 面特征点选择标志
float res_last[100000] = {0.0};                  // 上一次残差
int points_cache_size = 0;                       // 点云缓存大小

// 帧合并用于聚类提取
deque<PointCloudXYZI::Ptr> feats_world_sliding_window;  // 世界坐标系下的特征点滑动窗口

// 地图中的面特征点
PointCloudXYZI::Ptr featsFromMap(new PointCloudXYZI());                 // 从地图中提取的特征点
PointCloudXYZI::Ptr feats_undistort_lidar(new PointCloudXYZI());        // 去畸变后的雷达坐标系点云
PointCloudXYZI::Ptr feats_body_dense_filter_dynamic(new PointCloudXYZI());  // 过滤动态物体后的稠密点云
PointCloudXYZI::Ptr feats_undistort_orig_lidar(new PointCloudXYZI());   // 去畸变原始雷达点云
PointCloudXYZI::Ptr feats_down_lidar(new PointCloudXYZI());             // 降采样后的雷达点云
PointCloudXYZI::Ptr feats_down_world(new PointCloudXYZI());             // 降采样后的世界坐标点云
PointCloudXYZI::Ptr normvec(new PointCloudXYZI(100000, 1));             // 法向量点云
PointCloudXYZI::Ptr laserCloudOri(new PointCloudXYZI(100000, 1));       // 原始激光点云
PointCloudXYZI::Ptr corr_normvect(new PointCloudXYZI(100000, 1));       // 对应法向量
PointCloudXYZI::Ptr _featsArray;                                         // 特征数组

// 降采样滤波器
pcl::VoxelGrid<PointType> downSizeFilterSurf;  // 面特征降采样滤波器
pcl::VoxelGrid<PointType> downSizeFilterPCD;   // PCD降采样滤波器

// ikd-Tree全局地图
KD_TREE<PointType> ikdtree;                    // ikd-Tree动态KD树地图
V3F XAxisPoint_body(LIDAR_SP_LEN, 0.0, 0.0);   // 机体坐标系X轴点
V3F XAxisPoint_world(LIDAR_SP_LEN, 0.0, 0.0);  // 世界坐标系X轴点
V3D euler_cur;                                  // 当前欧拉角
V3D position_last(Zero3d);                      // 上一次位置

// 估计器输入和输出
MeasureGroup Measures;                          // 测量组
StatesGroup state;                              // 状态组
nav_msgs::Path path;                            // 路径消息
nav_msgs::Odometry odomAftMapped;               // 建图后的里程计
geometry_msgs::Quaternion geoQuat;              // 几何四元数
geometry_msgs::PoseStamped msg_body_pose;       // 机体位姿消息
shared_ptr<Preprocess> p_pre(new Preprocess()); // 预处理器
...
/**
 * @brief 将速度从雷达坐标系转换到重力对齐坐标系
 * @param L0_vel_L 雷达坐标系下的速度(输入/输出)
 */
void ConvertTwistToGravFrame(V3D &L0_vel_L) {
    M3D omg_hat;  // 角速度反对称矩阵
...
/**
 * @brief IMU状态传播一次
 * @param imu_prop_state IMU传播状态
 * @param dt 时间增量
 * @param acc_avr 平均加速度
 * @param angvel_avr 平均角速度
 */
...
/**
 * @brief 标准点云回调函数(Velodyne, Ouster, Pandar等)
 * @param msg 标准ROS PointCloud2消息
 */
...
/**
 * @brief 增量式地图更新,将新点云加入ikd-tree全局地图
 */
...
/**
 * @brief 主函数:Swarm-LIO2激光建图主程序
 * 该函数实现了多无人机协同的激光惯性里程计与建图系统
 */
...
//                cout << " -- [feats_undistort_lidar size]   " << feats_undistort_lidar->points.size() << endl;
            ...
            // ========== 迭代状态估计(ESIKF) ==========
            vector<int> teammateID_with_observation;           // 有观测的队友ID
            vector<int> teammateID_with_active_observation;    // 主动观测的队友ID
            vector<int> teammateID_with_passive_observation;   // 被动观测的队友ID
            ...
            // ========== ESIKF迭代开始 ==========
            bool degeneration_detected = false;  // 退化检测标志
        ...
        #pragma omp parallel for
        #endif
                for (int i = 0; i < feats_down_size; i++) {
                    PointType &point_lidar = feats_down_lidar->points[i];  // L_k系(雷达坐标系)
                    PointType &point_world = feats_down_world->points[i];  // World系(世界坐标系)
                    ...
                    pabcd.setZero();
                    // 拟合平面方程 ax+by+cz+d=0
                    if (esti_plane(pabcd, points_near, 0.1))  // 平面有效
                    {
                        // 计算点到平面的距离
                ...
                double smallest_singular_value = svd.singularValues()(2);  // 最小奇异值
                if (smallest_singular_value < degeneration_thresh) {
                    cout << BOLDYELLOW << "LiDAR Degeneration Detected!" << RESET << endl;
                    mutual_observe_noise = 0.00005;  // 退化时降低互相观测噪声
                    degeneration_detected = true;
                }


                // ========== 计算测量雅可比矩阵H和测量向量 ==========
                vector<Triplet<double>> tripletsHsub;       // H矩阵的三元组表示
                vector<Triplet<double>> tripletsHsubT;      // H^T * R^{-1}的三元组表示
                vector<Triplet<double>> triplets_meas_vec;  // 测量向量的三元组表示
