...
using namespace std;
using namespace fmt;
#define UDP_PORT 8821              // UDP通信端口号
#define BUFFER_SIZE 10086          // UDP接收缓冲区大小
#define OBSERVE_MSG_TYPE 0x04u     // 观测消息类型标识

using namespace udp_bridge;
typedef unordered_map<int, Teammate> id_teammate_map;  // 队友ID到Teammate对象的映射
typedef id_teammate_map::value_type position;          // 键值对类型
bool exit_process = false;  // 进程退出标志
#define DEBUG_FILE_DIR(name)     (string(string(ROOT_DIR) + "log/"+ name))


/**
 * @brief 信号处理函数 - 捕获退出信号并设置退出标志
 * @param sig 信号编号
 */
...
/**
 * @class UdpBridge
 * @brief UDP桥接类 - 负责集群无人机间的UDP通信、时间同步和状态共享
 */
class UdpBridge {
private:
    // UDP通信相关
    int udp_server_fd_;                      // UDP服务器文件描述符
    int udp_send_ip_fd_ptr_;                 // UDP发送IP的文件描述符
    char udp_recv_buf_[BUFFER_SIZE];         // UDP接收缓冲区
    string local_ip;                          // 本地IP地址
    int local_id;                             // 本地无人机ID
    int drone_state = -1;                     // 无人机状态
    id_teammate_map teammates;                // 队友列表映射表
    string broadcast_ip, offset_path;         // 广播IP地址和时间偏移保存路径
    ofstream save_offset;                     // 保存时间偏移的文件流

    // 网络地址和线程
    sockaddr_in addr_udp_send_ip_;            // UDP发送地址结构
    boost::thread *udp_callback_thread_;      // UDP回调线程

    // ROS相关
    ros::NodeHandle nh_;                      // ROS节点句柄
    ros::Publisher team_status_pub_;          // 队伍状态发布器
    ros::Timer sync_timer_, broadcast_timer_, debug_timer_, drone_state_timer_;  // 定时器
    ros::Publisher QuadState_pub_, GlobalExtrinsic_pub_, ST_OffsetStatus_pub_;  // 状态发布器
    ros::Subscriber QuadState_sub_, GlobalExtrinsic_sub_, Battery_sub_, DroneState_sub_, TeammateListTraj_sub_;  // 订阅器
    ros::Time udp_start_time_;                // UDP模块启动时间
    fstream log_writer_;                      // 日志写入流
    ofstream fout_delay;                      // 延迟信息输出流
    vector<int> teammate_id_by_traj_matching; // 通过轨迹匹配得到的队友ID列表
    mutex traj_buffer_mtx;                    // 轨迹缓冲区互斥锁
...
private:
    /**
     * @struct IpIdData
     * @brief IP和ID消息数据结构 - 用于处理队友IP和ID广播消息
     */
    struct IpIdData {
        ros::Timer process_timer;       // 处理定时器
        bool rcv_new_msg{false};        // 是否收到新消息标志
        mutex update_lock_;             // 更新锁
        IpIdMsgCvt latest_msg, processing_msg;  // 最新消息和处理中的消息
        ros::Time rcv_WT;               // 消息接收的系统时间
    } ip_id_data_;

    /**
     * @brief IP和ID消息回调函数 - 处理收到的队友IP和ID信息,发现新队友或更新连接状态
     * @param e 定时器事件
     */
...
private:
    /**
     * @struct TimeSyncData
     * @brief 时间同步消息数据结构 - 用于实现集群间的时钟同步
     */
    struct TimeSyncData {
        ros::Timer process_timer;       // 处理定时器
        bool rcv_new_msg{false};        // 是否收到新消息标志
        mutex update_lock_;             // 更新锁
        TimeSyncMsgCvt latest_msg, processing_msg;  // 最新消息和处理中的消息
        ros::Time rcv_WT;               // 消息接收的系统时间
    } time_sync_data_;

    /**
     * @brief 时间同步回调函数 - 基于双向消息传递计算时间偏移和网络延迟
     * 采用类似NTP协议的方法，通过t1,t2,t3,t4四个时间戳计算时间偏移
     * @param e 定时器事件
     */
    void TimeSyncCallback_(const ros::TimerEvent &e) {
        ...
        // 情况1: 如果client ID是自己，说明收到了服务器的回复消息
        if (rcv_client_id == local_id) {
            ...
            // 计算时间同步的四个时间戳 (类似NTP协议)
            // t1: 客户端发送时间, t2: 服务器接收时间
            // t3: 服务器发送时间, t4: 客户端接收时间
            ...
            double dt1 = (t2 - t1).toSec();  // 上行传输时间 + 时间偏移
            double dt2 = (t4 - t3).toSec();  // 下行传输时间 - 时间偏移

            // 计算时间偏移: offset = [(t2-t1) - (t4-t3)] / 2
            double offset_t = 0.5 * (dt1) - 0.5 * (dt2);
            // 计算网络延迟: delay = (t2-t1) + (t4-t3)
            double dt = (dt1) + (dt2);
        ...
        // 情况2: 如果client ID不是自己，说明自己作为服务器收到了同步请求
        auto iter = teammates.find(rcv_client_id);
        if (iter != teammates.end()) {
            auto &drone = iter->second;
            // 记录t2(接收时间)和t3(发送时间)，并将消息发回给客户端
...
///////////////////// 四旋翼状态消息处理 //////////////////////////////
     ...
     * @brief 四旋翼状态消息数据结构 - 包含位姿、速度、角速度等状态信息
     */
    struct QuadStateData {
        ros::Timer process_timer;       // 处理定时器
        bool rcv_new_msg{false};        // 是否收到新消息标志
        mutex update_lock_;             // 更新锁
        QuadStateCvt latest_msg, processing_msg;  // 最新消息和处理中的消息
        ros::Time rcv_WT;               // 消息接收的系统时间
    } quad_state_data_;

    /**
     * @brief 四旋翼状态回调函数 - 接收队友的状态信息并进行时间同步后发布
     * @param e 定时器事件
     */
...
///////////////////// 全局外参消息处理 //////////////////////////////
     ...
     * @brief 全局外参状态数据结构 - 用于多机协同定位中的坐标系变换关系
     */
    struct GlobalExtrinsicStatusData {
        ros::Timer process_timer;       // 处理定时器
        bool rcv_new_msg{false};        // 是否收到新消息标志
        mutex update_lock_;             // 更新锁
        GlobalExtrinsicStatusCvt latest_msg, processing_msg;  // 最新消息和处理中的消息
        ros::Time rcv_WT;               // 消息接收的系统时间
    } global_extrinsic_data_;

    /**
     * @brief 全局外参回调函数 - 接收并发布不同无人机间的坐标系变换关系
     * @param e 定时器事件
     */
     ...
     * @brief 地面站命令消息数据结构 - 用于接收和处理地面站发送的控制命令
     */
    struct GCSCmdMsgData {
        ros::Timer process_timer;       // 处理定时器
        bool rcv_new_msg{false};        // 是否收到新消息标志
        mutex update_lock_;             // 更新锁
        GCSCmdMsgCvt latest_msg, processing_msg;  // 最新消息和处理中的消息
        ros::Time rcv_WT;               // 消息接收的系统时间
    } GCSCmdMsg_data_;

    /**
     * @brief 地面站命令消息回调函数 - 根据命令执行相应的系统操作
     * 支持的命令包括: 启动驱动、启动SLAM、关闭节点、重启、关机、更新代码等
     * @param e 定时器事件
     */
...
//    }
    ...
    /**
     * @brief 初始化所有数据回调定时器 - 为各类消息创建处理定时器
     */
...
public:
    ...
    /**
     * @brief UdpBridge构造函数 - 初始化UDP通信、设置本地IP/ID、创建订阅器和发布器
     * @param nh ROS节点句柄
     */
...
//        debug_timer_ = nh_.createTimer(ros::Duration(0.001), &UdpBridge::DebugProcessCallback, this);
    ...
    /**
     * @brief UdpBridge析构函数 - 关闭文件描述符和日志文件
     */
    ~UdpBridge() {
    ...
    /**
     * @brief 广播回调函数 - 定期广播本地IP/ID并发布队伍状态
     * @param e 定时器事件
     */
...
//                   |             |               |
    ...
    /**
     * @brief 同步请求回调函数 - 向未完成时间同步的队友发送同步请求
     * @param e 定时器事件
     */
    ...
    /**
     * @brief UDP接收回调函数 - 持续接收UDP消息并根据类型分发处理
     * 在独立线程中运行，接收所有类型的UDP消息
     */
    void UdpCallback() {
        int valread;
        ...
        while (true) {
            ...
            ros::Time t2 = ros::Time::now();  // 记录接收时间
...
private:
    ...
    void CallSyncRequest(const Teammate &drone) {
        TimeSyncMsgCvt cvt;
        cvt.data.client_id = local_id;       // 设置客户端ID为本机ID
        cvt.data.server_id = drone.id_;      // 设置服务器ID为队友ID
        ros::Time t1 = ros::Time::now();     // 记录发送时间t1
        int len = sizeof(cvt.binary) + 2 * sizeof(uint32_t);
        char send_buf[len * 5];
...
//     print("Broadcast local Ip and Id.\n");
    ...
    /**
     * @brief 将消息编码到发送缓冲区
     * 消息格式: [消息类型ID(4字节)] [消息大小(4字节)] [消息内容]
     * @tparam union_msg 消息联合体类型
     * @param msg_type_id 消息类型标识符
     * @param msg 待编码的消息
     * @param send_buf_ 发送缓冲区
     * @return 编码后的消息长度
     */
    ...
    /**
     * @brief 初始化UDP单播套接字 - 为特定队友创建UDP发送端口
     * @param drone 目标队友对象
     * @param port UDP端口号
     * @return 套接字文件描述符
     */
    ...
    /**
     * @brief 初始化UDP广播套接字 - 用于向网络中所有节点广播消息
     * @param port UDP端口号
     * @return 套接字文件描述符
     */
    ...
    /**
     * @brief 绑定UDP套接字到指定端口 - 用于接收UDP消息
     * @param port UDP端口号
     * @param server_fd 服务器套接字文件描述符引用
     * @return 套接字文件描述符
     */
    ...
    /**
     * @brief 队友列表轨迹匹配回调函数 - 更新通过轨迹匹配连接的队友ID列表
     * @param msg 连接的队友列表消息
     */
    ...
    /**
     * @brief 全局外参ROS消息回调函数 - 接收本地外参消息并通过UDP发送给队友
     * 同时计算并发布时空偏移状态(包含重力坐标系下的外参)
     * @param msg 全局外参状态消息
     */
    ...
    /**
     * @brief 四旋翼状态ROS消息回调函数 - 接收本地状态消息并通过UDP发送给所有队友
     * @param msg 四旋翼状态发布消息
     */
    ...
    /**
     * @brief 无人机状态定时回调函数 - 定期向地面站发送无人机状态
     * @param e 定时器事件
     */
